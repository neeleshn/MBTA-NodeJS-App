app.controller("GreenECtrl", function ($scope, $sce, UserService, $http, $routeParams) {
    ...
    FavTrips = function () {
        ...
        for (var k = 0; k < $scope.ActiveUser.Trips.length; k++) {
            if ($scope.ActiveUser.Trips[k].Line == "GreenE")
                GreenETrips.push($scope.ActiveUser.Trips[k]);
        }

        angular.forEach(GreenETrips, function (eachTrip) {
            $http.get("http://realtime.mbta.com/developer/api/v2/schedulebyroute?api_key=TbUOod1u00WYU5oe6x7h1g&route=green-e&format=json").success(function (response) {
                ...
                $scope.FavGreenETrips.push("Next Train from " + FromStationjs + " is at " + FromTime + " and will reach " + ToStationjs + " at " + ToTime);
            });
        });
    }
    ...
    $scope.AddTrip = function () {
        ...
        var newtrip = { Start: $scope.FromStation, Stop: $scope.ToStation, Line: "GreenE" };
